...
#Hostname Config
sudo hostnamectl set-hostname awx-server.rbarozzi.com
privip=$(ip addr show eth0 | grep -w inet | awk '{ print $2; }' | cut -d/ -f1)
echo "$privip   awx-server.rbarozzi.com awx-server" | sudo tee -a /etc/hosts
echo "127.0.0.1 awx-server.rbarozzi.com  awx-server" | sudo tee -a /etc/hosts
...
sudo dnf install -y make git gcc gcc-c++ nodejs gettext device-mapper-persistent-data bzip2 python3-pip ansible
...
sudo lvcreate -L 100G -n awxlv awxvg
...
sudo echo "pg_hostname=pgsql.rbarozzi.com" >> inventory
